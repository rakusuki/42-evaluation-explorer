...
@router.get("/report/{login}")
async def probe_report(
    login: str,
    project: str | None = Query(
        None,
        description=(
            "Optional project ID or slug. Tests project and "
            "project-scales access."
        ),
    ),
) -> dict[str, Any]:
    """Run a sanitized multi-endpoint capability probe."""
    return await capability_report_service.build(
        login=login,
        project=project,
    )
...
async def probe_user_projects(login: str) -> dict[str, Any]:
    return await run_probe(
        lambda: evaluation_service.user_projects(login)
    )
...
async def probe_user_teams(login: str) -> dict[str, Any]:
    return await run_probe(
        lambda: evaluation_service.teams_for_user(login)
    )
...
async def probe_as_corrected(login: str) -> dict[str, Any]:
    return await run_probe(
        lambda: evaluation_service.scale_teams_as_corrected(login)
    )
...
async def probe_as_corrector(login: str) -> dict[str, Any]:
    return await run_probe(
        lambda: evaluation_service.scale_teams_as_corrector(login)
    )


@router.get("/projects/{project_id_or_slug}")
async def probe_project(
    project_id_or_slug: str,
) -> dict[str, Any]:
    return await run_probe(
        lambda: evaluation_service.project(project_id_or_slug)
    )


@router.get("/projects/{project_id_or_slug}/scales")
async def probe_project_scales(
    project_id_or_slug: str,
) -> dict[str, Any]:
    return await run_probe(
        lambda: evaluation_service.project_scales(
            project_id_or_slug
        )
    )
...
async def probe_scale_team(
    scale_team_id: int,
) -> dict[str, Any]:
    return await run_probe(
        lambda: evaluation_service.scale_team(scale_team_id)
    )


@router.get("/scale-teams/{scale_team_id}/feedbacks")
async def probe_feedbacks(
    scale_team_id: int,
) -> dict[str, Any]:
    return await run_probe(
        lambda: evaluation_service.scale_team_feedbacks(
            scale_team_id
        )
    )
...
async def probe_team_uploads(team_id: int) -> dict[str, Any]:
    return await run_probe(
        lambda: evaluation_service.team_uploads(team_id)
    )
...
async def probe_raw(
    resource: str = Query(
        ...,
        description="Whitelisted probe name",
    ),
    value: str = Query(
        ...,
        description="Login, ID, or slug for the probe",
    ),
) -> dict[str, Any]:
    probes: dict[str, Callable[[], Awaitable[Any]]] = {
        "user": lambda: evaluation_service.user(value),
        "user_projects": lambda: evaluation_service.user_projects(
            value
        ),
        "user_teams": lambda: evaluation_service.teams_for_user(
            value
        ),
        "as_corrected": lambda: (
            evaluation_service.scale_teams_as_corrected(value)
        ),
        "as_corrector": lambda: (
            evaluation_service.scale_teams_as_corrector(value)
        ),
        "project": lambda: evaluation_service.project(value),
        "project_scales": lambda: (
            evaluation_service.project_scales(value)
        ),
    }
    ...
    if call is None:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported probe: {resource}",
        )
